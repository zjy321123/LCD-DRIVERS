...
void Screen_80_Movie(unsigned int Keyid)
{
    ...
    {
        lv_event_send(guider_ui.screen_80_btn_2, LV_EVENT_SHORT_CLICKED, NULL); // 加载界面screen_12
        current_screen = SCREEN_12;
        return;
    }
    else if (Keyid == KEYENTER_2S)
    {
        lv_event_send(guider_ui.screen_80_btn_2, LV_EVENT_CLICKED, NULL); // 加载界面screen_13
